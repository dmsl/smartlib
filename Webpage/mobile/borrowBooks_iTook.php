...
function getMyRequests(){
	...
	/*
	 * VIVLIA POU CURRENTLY DANEIZOME APO ALLOUS

SELECT BI.isbn, BI.title, BI.authors, U.username, BR.date
FROM SMARTLIB_BORROWS BR, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI, SMARTLIB_USER U
WHERE 
B.B_ID=BR.B_ID AND B.BI_ID=BI.BI_ID
AND B.U_ID=U.U_ID
AND
BR.U_ID='105'
	...
	$queryFindPrevRequest= sprintf("SELECT BI.isbn, BI.title,  BI.imgURL, BI.authors, U.username, BR.date ".
			" FROM SMARTLIB_BORROWS BR, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI, SMARTLIB_USER U "
			. " WHERE B.B_ID=BR.B_ID AND B.BI_ID=BI.BI_ID AND B.U_ID=U.U_ID AND BR.U_ID='%s'".
			" ORDER BY U.username",
