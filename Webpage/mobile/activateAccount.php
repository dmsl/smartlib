...
{
    $actCodeQRes =
        mysql_query("SELECT activationCode, U_ID, name, level FROM SMARTLIB_USER WHERE username='" . $_SESSION['REGusername'] . "'");
    ...
    if ($level != "0") {
        if ($level == "-2") {
            $result = array(
                "result" => "0",
                "message" => "You are banned from SmartLib"
            );
        } else if ($level == "-1") {
            $result = array(
                "result" => "0",
                "message" => "You are visitor to SmartLib"
            );
        } else if ($level == "1") {
            $result = array(
                "result" => "0",
            ...
            $result = array(
                "result" => "0",
                "message" => "Account already activated<br>You are admin to SmartLib"
            );
        }
...
//			header("Location: ../registerSuccess.php");
        $result = array(
            "result" => "0",
            "message" => "Failed to activate user: Database Error :("
        );
