...
function getMyRequests(){
	...
	/*
	 SELECT BI.isbn, BI.title, BI.authors,OWNER.username, R.date, R.acknowledge
FROM SMARTLIB_REQUESTS R, SMARTLIB_USER U, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI,
SMARTLIB_USER OWNER
WHERE
R.B_ID=B.B_ID AND B.BI_ID= BI.BI_ID AND OWNER.U_ID=B.U_ID
AND
R.U_ID=U.U_ID AND U.username='pmpeis01';
	  
	 */

	$queryFindPrevRequest= sprintf("SELECT BI.isbn, BI.title, BI.authors,  BI.imgURL, OWNER.username, "
			." R.date, R.acknowledge "
			."FROM SMARTLIB_REQUESTS R, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI, "
			. " SMARTLIB_USER OWNER "
			." WHERE R.B_ID=B.B_ID AND B.BI_ID= BI.BI_ID AND OWNER.U_ID=B.U_ID "
			." AND R.U_ID='%s' ORDER BY R.acknowledge",
