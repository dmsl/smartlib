...
function getMyRequests(){
	...
	/* VIVLIA POU CURRENLTY DANEIZW EGW SE ALLOUS!!!!!


SELECT BI.title , BI.isbn, BI.authors, U2.username, BR.date
FROM SMARTLIB_BORROWS BR, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI, SMARTLIB_USER U,
SMARTLIB_USER U2
WHERE 
U.U_ID=B.U_ID  AND B.B_ID=BR.B_ID 
AND B.BI_ID=BI.BI_ID AND U2.U_ID=BR.U_ID
AND
U.U_ID='23'
	  
	 */

	$queryFindPrevRequest= sprintf("SELECT BI.title , BI.isbn, BI.authors,  BI.imgURL, U2.username, BR.date "
			." FROM SMARTLIB_BORROWS BR, SMARTLIB_BOOK B, SMARTLIB_BOOK_INFO BI, SMARTLIB_USER U, "
			." SMARTLIB_USER U2 WHERE U.U_ID=B.U_ID  AND B.B_ID=BR.B_ID AND B.BI_ID=BI.BI_ID AND "
			." U2.U_ID=BR.U_ID AND U.U_ID='%s' ORDER BY U2.username",
