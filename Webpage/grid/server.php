...
$sord = $_REQUEST['sord']; // get the sording order

$searchString = $_REQUEST['searchString']; // get the generic search string
...
if ($searchOn == 'true') {
    ...
    foreach ($sarr as $k => $v) {
        switch ($k) {
            case 'searchString':
                $wh .= " AND ( title LIKE '%" . $v . "%' ";
                $wh .= " OR authors LIKE '%" . $v . "%' ";

                if ($examp == 'allbooksloggedin')
                    $wh .= " OR username LIKE '%" . $v . "%' ";
...
switch ($examp) {
    ...
    case 'allbooks':
        $result = mysql_query("Select COUNT(*) AS count FROM SMARTLIB_BOOK_INFO  WHERE 1=1 $wh ");
		...
		BI.lang, BI.isbn FROM SMARTLIB_BOOK_INFO BI WHERE 1=1 $wh ORDER BY $sidx $sord LIMIT $start , $limit";
