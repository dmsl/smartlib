/*   GNU Public Licence
 *   BooksGiven List of currently given books.
 *
 ...
 *
 *  @file BooksGiven.m
 *  @brief List of currently given books.
 *
...
- (void)viewDidLoad
{
    ...
    BooksTable =(NSMutableArray*)[action BooksGivenFromUser:username];
    [BooksTable retain];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    if (result == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No Books!" message:@"You have not given any book yet." delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
        [alert show];
        [alert release];
...
{
    static NSString *CellIdentifier = @"Given";
...
{    
    ...
    NSString *borrower = [[BooksTable objectAtIndex:indexPath.row]objectForKey:@"username"];
    NSString *when = [[BooksTable objectAtIndex:indexPath.row]objectForKey:@"date"];
    NSString *msg =[[NSString alloc]initWithFormat:@"To: %@ \n Date: %@ ", borrower,when];
